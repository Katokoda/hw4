...
Injecting $\mu^* = \left( \frac{\lambda_{\min} (A)}{2}, \frac{\lambda_{\min} (B)}{2}, 0)^\top\right)$, this gives:
...
At this point, we need to recall that $(x^*,y^*)$ must belong to the set $S$, or equivalently we must have $h(x^*,y^*)=0$. By taking unit-normed eigenvectors, we ensure that $h_1(x^*,y^*)=0=h_2(x^*,y^*)$. The last condition is $h_3(x^*,y^*) = 0$. By definition of $h_3$, this means $(x^*)^\top y^* = 0$, i.e. $x^*$ and $y^*$ need to be orthogonal in $\R^n$. This is the only assumption we do not know, but observe that as soon as we know it the previous reasoning implies that the assumptions of the strong duality theorem are satisfied.\\
...
&=2\mu_1 x+\mu_3 y
\end{align*}
As $\p(x^\top x)=\p \sum_{i=1}^n x_i^2=2x_i$ and  $\p(x^\top y)=\p \sum_{i=1}^n x_i y_i=y_i$\\

Now for the third part we will use the fact that $\nabla||x||^2=2x$ as $x^\top x=||x||^2$ and we've just computed it before. Plus we know the chain rule: $\p f\circ g(x)= \nabla f (g(x)) \cdot \p g(x)$. We can use it to compute our $\nabla$ :
\begin{align*}
...
&=\beta\begin{bmatrix} -2x & \vec 0 & y \end{bmatrix}   \cdot \begin{bmatrix} 1-x^\top x \\ 1-y^\top y\\ x^\top y \end{bmatrix} \\
&=\beta\begin{bmatrix}2(x^\top x-1) x_1+(x^\top y )y_1\\ \dots \\ 2(x^\top x-1) x_n+(x^\top y )y_n \end{bmatrix}\\
&=\beta \left( 2(x^\top x-1) \vec x+(x^\top y )\vec y \right) 
\end{align*}

Putting together our three calculations, we get
\begin{align*}
\nabla_x L_\beta(x,y,\mu)&=Ax+2\mu_1 x+\mu_3 y+\beta \left( 2(x^\top x-1) \vec x+(x^\top y )\vec y \right) \\
&=(A+ 2\beta (x^\top x-1)+2 \mu_1) x +(x^\top y + \mu_3)y
\end{align*}

Similarly, we get
\begin{align*}
\nabla_y L_\beta(x,y,\mu)&=By+2\mu_2 y+\mu_3 x+\beta \left( 2(y^\top y-1) \vec y+(x^\top y )\vec x \right) \\
&=(x^\top y + \mu_3)x+(B+ 2\beta (y^\top y-1)+2 \mu_2) y
\end{align*}
