...
In fact, this is the optimal value even for the whole condition $M_\mu\succeq 0$. Indeed, by question 2, %TODO what?
...
Note that the optimal value for (D) is bounded by $\frac{\lambda_min(A) + \lambda_min(B)}{2}$ = , therefore there is no $\mu$ that will show that our found $(x, y)$ is a global minimum. %TODO compléter la valeur, qui doit être calculée par MatLab... (ou Octave, si jamais)
