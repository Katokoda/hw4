...
Those functions use the implemented functions \mcode{f} and \mcode{h} which are a direct implementation of their definition.

\section*{Question 9}

We have used the Matlab \mcode{fminunc} function. !!TODO!! Think whether to use 'Quasi-Newton' or 'Trust-region', maybe with algorithmic time?
We have created a function \mcode{minXY} which calls the precedent function with the good parameters.\\
...
Note that the results are a little bit different if we are using 'Trust-region'. !!TODO!!
...
We see that after 9 iterations, the points $x$ and $y$ still \textit{far} from $h(x, y) = 0$. That's why we consider the augmented Lagrangian method.
...
Note that the only difference is the update of $\mu$ at line 10.
The final iteration leaves:
\begin{lstlisting}
Iteration 9 with beta = 256.
We found x and y:
